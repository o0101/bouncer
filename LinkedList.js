    ...
    // getter and setter for end node
      ...
      get end() {
        ...
        while( !! link && !! link.next ) {
          if ( link.next == this.head ) { // circular check (already at last)
            break;
          }
          link = link.next;
        }
    ...
    // find a link before a target link
      findBefore( targetLink ) {
        ...
        while( !! link && link !== targetLink ) {
          previousLink = link;
          link = link.next;
          if ( link == this.head ) { // circular check
            break;
          }
        }
        if ( link == targetLink ) {
          ...
          throw new TypeError(`No link before given targetLink with data: ${targetLink.data}`);
        }
      }
